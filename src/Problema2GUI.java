...
public class Problema2GUI extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {// aici e actiunea butoanelor
        ...
        if (e.getSource() == rezButton) {
            ...
            int dMAB = problema2.Determinant(problema2.m[0], problema2.x[0], problema2.x[1], problema2.m[1],
                    problema2.y[0], problema2.y[1]);
            int dMBC = problema2.Determinant(problema2.m[0], problema2.x[1], problema2.x[2], problema2.m[1],
                    problema2.y[1], problema2.y[2]);
            int dMCA = problema2.Determinant(problema2.m[0], problema2.x[2], problema2.x[0], problema2.m[1],
                    problema2.y[2], problema2.y[0]);
            if (problema2.Determinant(problema2.x[0], problema2.x[1], problema2.x[2], problema2.y[0], problema2.y[2],
                    problema2.x[2]) == 0) {
                JOptionPane.showMessageDialog(null, "Cele trei puncte sunt coliniare");
            } else if (problema2.Determinant(problema2.x[0], problema2.x[1], problema2.x[2], problema2.y[0],
                    problema2.y[2], problema2.x[2]) > 0) {
                problema2.CazAbcMare(dMAB, dMBC, dMCA);
            } else
                problema2.CazAbcMic(dMAB, dMBC, dMCA);
        }
